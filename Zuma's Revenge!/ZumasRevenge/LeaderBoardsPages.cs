...
public class LeaderBoardsPages : Widget
{
	...
	{
		mLeaderBoards = theLeaderBoards;
		mNumPages = 0;
		mHeaderFont = Res.GetFontByID(ResID.FONT_SHAGLOUNGE45_GAUNTLET);
		mStatsFont = Res.GetFontByID(ResID.FONT_SHAGLOUNGE38_STROKE);
		mText = new List<LeaderBoardText>();
	...
	{
		int theStartY = (IMAGE_UI_LEADERBOARDS_SHADOW.GetHeight() - 40) * (mNumPages + 1);
		SetupLeaderboardsTextXLive(ref theStartY, page, isUpdate, reader);
		if (!isUpdate)
		{
			mNumPages++;
		}
	...
	{
		int theStartY = (IMAGE_UI_LEADERBOARDS_SHADOW.GetHeight() - 40) * (mNumPages + 1);
		SetupLeaderboardsText(ref theStartY, page, isUpdate);
		if (!isUpdate)
		{
			mNumPages++;
		}
	...
	public override void Draw(Graphics g)
	{
		...
		for (int i = 0; i < mText.Count; i++)
		{
			LeaderBoardText leaderBoardText = mText[i];
			int num2 = 0;
			int num3 = 0;
			...
			if (leaderBoardText.mValueStr.Length == 0)
			{
				...
				case 2:
				{
					...
					int num5 = 30;
					Rect theRect = new Rect(num2 + leaderBoardText.mX + (int)mLeaderBoards.mXOff, leaderBoardText.mY + g.GetFont().GetAscent() - num5, 250, mStatsFont.GetHeight() * 2);
				...
				{
					int num4 = 170;
					g.DrawString(leaderBoardText.mHeaderStr, num2 + leaderBoardText.mX + (int)mLeaderBoards.mXOff + num4 - g.GetFont().StringWidth(leaderBoardText.mHeaderStr), num3 + leaderBoardText.mY + g.GetFont().GetAscent());
					break;
				}
				default:
					g.DrawString(leaderBoardText.mHeaderStr, num2 + leaderBoardText.mX + (int)mLeaderBoards.mXOff, num3 + leaderBoardText.mY + g.GetFont().GetAscent());
					break;
				}
				if ((i + 1) % 3 == 0)
				{
			...
			else
			{
				...
				g.WriteString(leaderBoardText.mHeaderStr, num2 + leaderBoardText.mX + (int)mLeaderBoards.mXOff, num3 + leaderBoardText.mY + mStatsFont.GetAscent(), 0, 1);
				g.SetColor(89, 187, 149);
				g.WriteString(leaderBoardText.mValueStr, num2 + leaderBoardText.mX + Common._DS(Common._M(20)) + (int)mLeaderBoards.mXOff, num3 + leaderBoardText.mY + mStatsFont.GetAscent(), mWidth, -1);
			}
		}
		int num6 = NumPages() - 1;
		int num7 = IMAGE_UI_CHALLENGESCREEN_TAPESTRY_LEAVES.GetWidth() + 30;
		int theY = (IMAGE_UI_CHALLENGESCREEN_TAPESTRY_LEAVES.GetHeight() - IMAGE_UI_CHALLENGESCREEN_DIVIDER.GetHeight()) / 2;
		for (int j = 0; j < num6; j++)
		{
			g.DrawImage(IMAGE_UI_CHALLENGESCREEN_DIVIDER, num7 - IMAGE_UI_CHALLENGESCREEN_DIVIDER.GetWidth(), theY);
			num7 += IMAGE_UI_CHALLENGESCREEN_TAPESTRY_LEAVES.GetWidth() + 30;
		}
	...
	private void SetupLeaderboardsTextXLive(ref int theStartY, int page, bool update, LeaderboardReader reader)
	{
		...
		int num2 = 40 + mStatsFont.mHeight;
		...
		if (!update)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				mText.Add(new LeaderBoardText());
				leaderBoardText = mText.back();
				...
				try
				{
					...
					leaderBoardText.mIcon = GameApp.gApp.mAppDriver.GetOptimizedImage(gamerPicture, commitBits: true, allowTriReps: true);
				}
				catch (Exception)
				{
					leaderBoardText.mIcon = Res.GetImageByID(ResID.IMAGE_UI_AVATAR);
		...
		for (int j = 0; j < count; j++)
		{
			...
			leaderBoardText = mText[j * 3];
			leaderBoardText.mHeaderStr = " " + (1 + j + reader.PageStart);
			leaderBoardText.mValueStr = "";
			leaderBoardText = mText[j * 3 + 1];
			leaderBoardText.mShowIcon = true;
			if (leaderBoardText.mIcon != null)
			{
			...
			try
			{
				...
				leaderBoardText.mIcon = GameApp.gApp.mAppDriver.GetOptimizedImage(gamerPicture2, commitBits: true, allowTriReps: true);
			}
			catch (Exception)
			{
				leaderBoardText.mIcon = Res.GetImageByID(ResID.IMAGE_UI_AVATAR);
	...
	public void UpdatePage(int page)
	{
		...
		{
			leaderBoardText = mText[i * 3];
			leaderBoardText.mHeaderStr = " " + (i + 1 + page * mNumLines);
			leaderBoardText.mValueStr = "";
			leaderBoardText = mText[i * 3 + 1];
			if (leaderBoardText.mIcon != null)
			{
	...
	private void SetupLeaderboardsText(ref int theStartY, int page, bool update)
	{
		...
		{
			leaderBoardText = mText[j * 3];
			leaderBoardText.mHeaderStr = " " + (j + 1 + page * mNumLines);
			leaderBoardText.mValueStr = "";
			leaderBoardText = mText[j * 3 + 1];
			if (leaderBoardText.mIcon != null)
			{
