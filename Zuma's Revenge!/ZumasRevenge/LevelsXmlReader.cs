...
public class LevelsXmlReader : ContentTypeReader<LevelMgr>
{
	...
	{
		LevelMgr levelMgr = (mLevelMgr = new LevelMgr());
		ReadLevels(input, levelMgr);
		input.ReadString();
		return levelMgr;
	}
	...
	private void ReadLevels(ContentReader input, LevelMgr mgr)
	{
		readDot(input, mgr);
		readDDS(input, mgr);
		readHandheldBalance(input, mgr);
		readGauntletMode(input, mgr);
		readTips(input, mgr);
		readScoreTip(input, mgr);
		readZone(input, mgr);
		readDefaults(input, mgr);
		int num = input.ReadInt32();
		Level level = null;
		for (int i = 0; i < num; i++)
		{
			level = new Level();
			readLevel(input, level);
	...
	private void readLevel(ContentReader input, Level level)
	{
		...
		for (int j = 0; j < num2; j++)
		{
			...
			{
				curveMgr = (level.mCurveMgr[num3] = new CurveMgr(null, num3));
				curveMgr.mLevel = level;
				if (mLevelMgr.mIsHardConfig)
				{
					text += "_hard";
				}
		...
		readTreasurePoint(input, level);
		readEffectParams(input, level);
		readTunnel(input, level);
		readGun(input, level);
		readPowerupRegion(input, level);
		readTorch(input, level);
		readWall(input, level);
		readMovingWall(input, level);
		readBoss(input, level);
	}
	...
	private void readScoreTip(ContentReader input, LevelMgr mgr)
	{
		...
		for (int i = 0; i < num; i++)
		{
			ScoreTip item = default(ScoreTip);
			mgr.mScoreTips.Add(item);
			item.mMinLevel = input.ReadInt32();
			item.mTipId = input.ReadInt32();
			item.mTip = TextManager.getInstance().getString(item.mTipId);
		}
	}
	...
	{
		int num = input.ReadInt32();
		TreasurePoint treasurePoint = null;
		for (int i = 0; i < num; i++)
		{
			treasurePoint = new TreasurePoint();
	...
	{
		int num = input.ReadInt32();
		PowerupRegion powerupRegion = null;
		for (int i = 0; i < num; i++)
		{
			powerupRegion = new PowerupRegion();
	...
	private void readMovingWall(ContentReader input, Level level)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			wall.mX = input.ReadInt32();
			wall.mY = input.ReadInt32();
			wall.mWidth = input.ReadInt32();
			wall.mHeight = input.ReadInt32();
			wall.mColor = new Color(input.ReadInt32());
			wall.mMinRespawnTimer = input.ReadInt32();
			wall.mMaxRespawnTimer = input.ReadInt32();
			wall.mMinLifeTimer = input.ReadInt32();
	...
	private void readBoss(ContentReader input, Level level)
	{
		...
		if (num > 0)
		{
			level.mBoss = readBossDefine(input, level);
			if (num > 1)
			{
				level.mSecondaryBoss = readBossDefine(input, level);
			}
		}
	}
	...
	private Boss readBossDefine(ContentReader input, Level level)
	{
		...
		int num = input.ReadInt32();
		BossWall bossWall = null;
		for (int i = 0; i < num; i++)
		{
			bossWall = new BossWall();
		...
		bossShoot.SetX(input.ReadInt32());
		bossShoot.SetY(input.ReadInt32());
		...
		readBossDDS(input, boss, level);
		readBossBerserk(input, boss);
		readBossSkeletonEmitter(input, boss);
		readBossHintText(input, boss);
		readBossHula(input, boss);
		readDeathText(input, boss);
		return boss;
	}
	...
	private void readBossDDS(ContentReader input, Boss boss, Level level)
	{
		...
		boss_DDS_Vals = (bossParams.ContainsKey(mId) ? bossParams[mId] : (bossParams[mId] = new Boss_DDS_Vals()));
		List<Boss_Param_Range> list = null;
		Boss_Param_Range boss_Param_Range = null;
		for (int i = 0; i < num; i++)
		{
			string key = input.ReadString();
			list = new List<Boss_Param_Range>();
			int num2 = input.ReadInt32();
			for (int j = 0; j < num2; j++)
			{
				boss_Param_Range = new Boss_Param_Range();
				boss_Param_Range.mMin = input.ReadSingle();
				boss_Param_Range.mMax = input.ReadSingle();
				boss_Param_Range.mRatingMin = input.ReadSingle();
				boss_Param_Range.mRatingMax = input.ReadSingle();
				list.Add(boss_Param_Range);
			}
			boss_DDS_Vals.mParams.Add(key, list);
		}
	}
	...
	private void readBossBerserk(ContentReader input, Boss boss)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			for (int j = 0; j < num2; j++)
			{
				...
				BerserkModifier item = new BerserkModifier(p, value, minval, maxval, flag);
				berserkTier.mParams.Add(item);
			}
		}
		BossShoot bossShoot = boss as BossShoot;
		num = input.ReadInt32();
		BossBerserkMovement bossBerserkMovement = null;
		Point point = null;
		for (int k = 0; k < num; k++)
		{
			bossBerserkMovement = new BossBerserkMovement();
	...
	{
		int num = input.ReadInt32();
		HulaEntry hulaEntry = null;
		for (int i = 0; i < num; i++)
		{
			hulaEntry = new HulaEntry();
	...
	{
		int num = input.ReadInt32();
		BossText bossText = null;
		for (int i = 0; i < num; i++)
		{
			bossText = new BossText();
