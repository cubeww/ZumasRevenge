...
public class GenericHelp : DialogEx, IDisposable
{
	...
	public GenericHelp()
		: base(null, null, 9, isModal: true, "", "", "", 0)
	{
		...
		Resize((GameApp.gApp.mWidth - num) / 2, (GameApp.gApp.mHeight - num2) / 2, num, num2);
		...
		mOKBtn.SetFont(fontByID);
		mOKBtn.Resize((mWidth - num3) / 2, Common._DS(Common._M1(900)), num3, theHeight);
		AddWidget(mOKBtn);
		...
		int[] array3 = new int[6] { 3, 4, 2, 1, 5, 6 };
		...
		{
			mHelpItems.Add(new GenericHelpItem());
			GenericHelpItem genericHelpItem = mHelpItems.Last();
	...
	public override void Draw(Graphics g)
	{
		...
		g.WriteString(TextManager.getInstance().getString(473), 0, Common._DS(Common._M(130)), mWidth, 0);
		...
		for (int i = 0; i < mHelpItems.Count(); i++)
		{
			...
			g.WriteWordWrapped(theRect, mHelpItems[i].mText, -1, -1);
			g.SetFont(fontByID3);
			g.SetColor(205, 151, 57);
			g.DrawString(mHelpItems[i].mHeader, theRect.mX + Common._DS(Common._M(0)), num7 + Common._DS(Common._M1(54)));
			int num9 = num8 + Common._DS(Common._M(20));
			int num10 = num7 + (num6 - mHelpItems[i].mImage.GetCelHeight()) / 2 + Common._DS(Common._M(0));
			...
			g.DrawImageCel(mHelpItems[i].mImage, num9, num10, mHelpItems[i].mCel);
		}
	}
