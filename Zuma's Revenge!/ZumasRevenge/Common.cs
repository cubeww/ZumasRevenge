...
public static class Common
{
	...
	public static int[] gBallColors = new int[6] { 1671423, 16776960, 16711680, 65280, 16711935, 16777215 };

	public static int[] gBrightBallColors = new int[6] { 8454143, 16777024, 16755370, 8454016, 16744703, 16777215 };

	public static int[] gDarkBallColors = new int[6] { 2299513, 6312202, 10489620, 2114594, 5641795, 3676962 };

	public static int[] gTextBallColors = new int[6] { 2984959, 16776960, 16711680, 65280, 16711935, 16777215 };

	public static bool IsDeprecatedPowerUp(PowerType ptype)
	{
		if (ptype != PowerType.PowerType_Fireball && ptype != PowerType.PowerType_ShieldFrog && ptype != PowerType.PowerType_FreezeBoss && ptype != PowerType.PowerType_BallEater && ptype != PowerType.PowerType_BombBullet)
		{
			return ptype == PowerType.PowerType_Lob;
		}
		return true;
	}
	...
	{
		return StrEquals(str1, str2, pIgnoreCase: true);
	}
	...
	public static void SetupDialog(Dialog theDialog)
	{
		...
		SetupDialogButton(theDialog.mYesButton);
		SetupDialogButton(theDialog.mNoButton);
	}
	...
	public static DialogButton MakeButton(int theId, ButtonListener theListener, string theText)
	{
		...
		SetupDialogButton(dialogButton);
		return dialogButton;
	}
	...
	public static void DrawCommonDialogBacking(Graphics g, int x, int y, int width, int height)
	{
		...
		int i = y;
		bool flag = false;
		for (; i <= y + height + imageByID.GetHeight(); i += imageByID.GetHeight())
		{
			while (num < x + width + imageByID.GetWidth())
			{
				if (flag)
				{
		...
		for (int j = x + imageByID4.GetWidth(); j < x + width - imageByID4.GetWidth(); j += imageByID3.GetWidth())
		{
			g.DrawImage(imageByID3, j, y - 1);
			g.DrawImage(imageByID3, j, y + height - imageByID3.GetHeight() + 1);
		}
		g.ClearClipRect();
		g.SetClipRect(x, y + imageByID4.GetHeight(), width, height - imageByID4.GetHeight() * 2);
		for (int k = y + imageByID4.GetHeight(); k < y + height - imageByID4.GetHeight(); k += imageByID2.GetHeight())
		{
			g.DrawImage(imageByID2, x, k);
			g.DrawImage(imageByID2, x + width - imageByID2.GetWidth(), k);
		}
		g.ClearClipRect();
	}
	...
	{
		return ((GameApp)GlobalMembers.gSexyApp).GetBoard()?.GetStateCount() ?? 0;
	}

	public static uint GetBoardTickCount()
	{
		return (uint)(GetBoardStateCount() * 10);
	}
	...
	public static bool BossLevel(Level level)
	{
		...
		if (!level.IsFinalBossLevel() && level.mBoss == null)
		{
			return level.mEndSequence > 0;
		}
		return true;
	}
	...
	{
		return LinesIntersect(a1, a2, b1, b2, null);
	}
	...
	public static bool LinesIntersect(FPoint a1, FPoint a2, FPoint b1, FPoint b2, FPoint intersectFPoint)
	{
		...
		b2.mX -= a1.mX;
		b2.mY -= a1.mY;
		double num = Math.Sqrt(a2.mX * a2.mX + a2.mY * a2.mY);
		...
		double num5 = b2.mX + (b1.mX - b2.mX) * b2.mY / (b2.mY - b1.mY);
	...
	{
		if (theRad >= 0f && theRad < (float)Math.PI * 2f)
		{
			return theRad;
		}
		return AceModF(theRad, (float)Math.PI * 2f);
	}
	...
	{
		return GameApp.gApp.mResourceManager.LoadImage(name)?.GetImage();
	}
	...
	public static void SerializePIEffect(PIEffect s, DataSync sync)
	{
		...
		buffer2.WriteLong(buffer.GetDataLen());
	...
	public static void DeserializePIEffect(PIEffect s, DataSync sync)
	{
		...
		byte[] theData = new byte[num];
		buffer.ReadBytes(ref theData, num);
		SexyFramework.Misc.Buffer buffer2 = new SexyFramework.Misc.Buffer();
		buffer2.SetData(theData, num);
		s.LoadState(buffer2);
	}
	...
	{
		SexyFramework.Misc.Buffer buffer = new SexyFramework.Misc.Buffer();
		s.Serialize(buffer, PILGetIDByImage);
		SexyFramework.Misc.Buffer buffer2 = sync.GetBuffer();
		buffer2.WriteLong(buffer.GetDataLen());
		buffer2.WriteBytes(buffer.GetDataPtr(), buffer.GetDataLen());
	}
	...
	public static SexyFramework.PIL.System DeserializeParticleSystem(DataSync sync)
	{
		...
		SexyFramework.Misc.Buffer buffer2 = new SexyFramework.Misc.Buffer();
		buffer2.SetData(theData, num);
		SexyFramework.PIL.System system = SexyFramework.PIL.System.Deserialize(buffer2, PILGetImageByID);
		system.mScale = _S(1f);
		return system;
	}
}
