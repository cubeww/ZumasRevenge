...
public class AchievementManager
{
	...
	private void Init()
	{
		...
		m_AchievementList[0] = achievementEntry;
		...
		m_AchievementList[1] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[2] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[3] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[4] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[5] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[6] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[7] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[8] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[9] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[10] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[11] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[12] = achievementEntry;
		achievementEntry = new AchievementEntry();
		...
		m_AchievementList[13] = achievementEntry;
	}

	public void getAchievementsInfo(ref int unlockedNum, ref int totalNum, ref int unlockedGPoint, ref int totalGPoint)
	{
		totalNum = m_AchievementList.Length;
		unlockedNum = 0;
		unlockedGPoint = 0;
		totalGPoint = 0;
		AchievementEntry[] achievementList = m_AchievementList;
		foreach (AchievementEntry achievementEntry in achievementList)
		{
	...
	{
		return m_AchievementList[(int)type];
	}

	public bool isAchievementUnlocked(EAchievementType type)
	{
		return m_AchievementList[(int)type].m_Unlocked;
	}

	public void UnlockAchievement(EAchievementType type)
	{
		m_AchievementList[(int)type].m_Unlocked = true;
		if ((GameApp.USE_XBOX_SERVICE || GameApp.UN_UPDATE_VERSION) && !GameApp.USE_TRIAL_VERSION && !UnlockAchievementXLive(type.ToString(), type))
		{
			ToggleAchievement(type);
		}
	}
	...
	public void Sync(DataSync sync)
	{
		for (int i = 0; i < 14; i++)
		{
			sync.SyncBoolean(ref m_AchievementList[i].m_Unlocked);
		}
	}
	...
	public bool UnlockAchievementXLive(string achievementKey, EAchievementType type)
	{
		...
		try
		{
			...
			signedInGamer.BeginAwardAchievement(achievementKey, AwardAchievementCallback, achievementUpdateEntry);
		}
		catch (Exception)
		{
			return false;
	...
	protected void AwardAchievementCallback(IAsyncResult result)
	{
		...
		{
			ToggleAchievement(achievementUpdateEntry.mType);
		}
	}

	public void SyncAchievementsXLive()
	{
		if (m_AchievementsXLive == null)
		{
			return;
		}
		foreach (Microsoft.Xna.Framework.GamerServices.Achievement item in m_AchievementsXLive)
		{
			EAchievementType eAchievementType = (EAchievementType)Enum.Parse(typeof(EAchievementType), item.Key, ignoreCase: false);
			if (item.IsEarned)
			{
				m_AchievementList[(int)eAchievementType].m_Unlocked = true;
			}
			else if (m_AchievementList[(int)eAchievementType].m_Unlocked)
			{
				UnlockAchievementXLive(item.Key, eAchievementType);
			}
		}
	}
}
