...
{
	private readonly GameApp SexyZuma;
	...
	private readonly SexyAppBase.Touch touch = new SexyAppBase.Touch();
	...
	private bool TryGetPointerLocation(int rawX, int rawY, bool clampToDisplayRect, out SexyFramework.Misc.Point point)
	{
		...
		{
			num = Math.Clamp(num, rectangle.Left, rectangle.Right - 1);
			num2 = Math.Clamp(num2, rectangle.Top, rectangle.Bottom - 1);
		}
		int num3 = 800;
		int num4 = 480;
		if (SexyZuma != null && SexyZuma.mWidgetManager != null)
		{
			Rect screenRect = SexyZuma.GetScreenRect();
			num3 = Math.Max(screenRect.mWidth, 1);
			num4 = Math.Max(screenRect.mHeight, 1);
		}
		float x2 = (float)(num - rectangle.X) * (float)num3 / (float)width;
		float y2 = (float)(num2 - rectangle.Y) * (float)num4 / (float)height;
		point = new SexyFramework.Misc.Point((int)Math.Clamp(x2, 0f, num3 - 1f), (int)Math.Clamp(y2, 0f, num4 - 1f));
		return true;
	}
	...
	private int GetViewportWidth()
	{
		...
		{
			num = Math.Max(base.GraphicsDevice.PresentationParameters.BackBufferWidth, 0);
		}
		int num2 = Math.Max(base.Window.ClientBounds.Width, 0);
		int num3 = GetPhysicalDisplayWidth();
		return Math.Max(Math.Max(num, num2), Math.Max(num3, 1));
	}
	...
	private int GetViewportHeight()
	{
		...
		if (base.GraphicsDevice != null)
		{
			num = Math.Max(base.GraphicsDevice.PresentationParameters.BackBufferHeight, 0);
		}
		int num2 = Math.Max(base.Window.ClientBounds.Height, 0);
		int num3 = GetPhysicalDisplayHeight();
		return Math.Max(Math.Max(num, num2), Math.Max(num3, 1));
	}

	private int GetPhysicalDisplayWidth()
	{
		Android.Util.DisplayMetrics? displayMetrics = Application.Context?.Resources?.DisplayMetrics;
	...
	private int GetPhysicalDisplayHeight()
	{
		Android.Util.DisplayMetrics? displayMetrics = Application.Context?.Resources?.DisplayMetrics;
		if (displayMetrics == null)
		{
